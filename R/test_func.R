...
#' @param 
get_coeffs <- function(graphs, graphs.by = 1, n1 = 50, km = 4, s = 1, h = 0.1, semicircle_real = NULL, rhos = c(1), alphas = c(1), rmax = 1, r.stepwise = 0.005) {
...
test_lv2_press <- function(coeffs, graphs, extension = 10) {
  graphs_xstars <- ddply(coeffs, .variables = .(id), function(coeff) {
    ...
    meanfieldMutual <- MeanFieldMutual$new(n1 = n1, s = s, c = c, km = km, m = m, h = h, graphm = graphm)
    ...
    ldply(out, function(one) {
      xstars <- one$xstars
      params <- one$params
      c(s = s, c = c, m = m, h = h, rho = rho, alpha = alpha, Delta = Delta, rmin = rmin, rmax = rmax, r.steps = r.steps, graphs_index = graphs_index, get_stability_from_params_xstars(params, xstars))
    })
  })
